...
func main() {
	...
	cacheTimeout := flag.Duration("cache-timeout", 10*time.Second, "transaction cache timeout")
	...
	// TODO: we may want to tune the cache here for size, etc.
	cache := ttlcache.New(
		ttlcache.WithTTL[string, string](*cacheTimeout),
	)
	...
	envoy_service_extproc_v3.RegisterExternalProcessorServer(gs, waf.NewExtProc(wafInstance, cache))
